...
void bbq_main_window::set_layout(bbq_layout layout) {
	 ...
	 */
	/*
	 * THE FRESHNESS LINE GETS ITS OWN ROW, IN BOTH SHAPES (sec 16.9).
	 *
	 * This is where a band failure is said, and sec 2.4 will not have a
	 * failed band fail invisibly. It was inside the control pane, and
	 * in both shapes that made it the least reachable thing on screen
	 * for a different reason: on mobile the last row of a scrolled
	 * pane, behind seven controls nobody scrolls past, and on the
	 * desktop the far end of a row that does not fit -- at the default
	 * 820 pixels it was clipped off the right edge entirely, so the
	 * shape most likely to be run was the one that never showed it.
	 *
	 * Verdict, graph, this, controls. Removed before inserting, because
	 * set_layout runs again on every rotation and unfold, and inserting
	 * a widget already in a layout is how a second slot appears.
	 *
	 * The margin is the controls' own: the root layout runs edge to
	 * edge on mobile because the PLOT wants every pixel, and a label is
	 * not plot. Zero on the desktop, where the root margins already
	 * supply it.
	 */
...
void bbq_main_window::begin(const QString &station_id, const QString &geocode) {
	...
	/*
	 * STILL NOT SCHEDULED, AND THE REASON HAS MOVED (sec 17.4).
	 *
	 * The foreground service works: the notification posts, the claim
	 * is accepted -- dumpsys shows isForeground=true with the channel
	 * -- and the ANR that killed the background one is gone. What does
	 * not happen is Qt: the loader never returns from onCreate and
	 * main() is never reached, so the fetch never runs.
	 *
	 * That is below anything here, in Qt's own Android service startup.
	 * Scheduling meanwhile would post a notification every fifteen
	 * minutes for a service that fetches nothing, which is worse than
