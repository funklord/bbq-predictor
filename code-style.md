...
Qt's API is `camelCase` and stays that way at the call site:
`setWindowTitle`, `paintEvent`, `addWidget`. That is the API's spelling,
not a violation.

Names this project introduces are `snake_case`, and carry the `bbq_`
prefix where they reach the linker -- which for a Qt application means
every class, since a class with `Q_OBJECT` has external symbols by
construction and moc generates more. So `bbq_main_window`, not
`main_window` and not `MainWindow`. The same convention beerssh uses with
`bssh_`.

Overriding a Qt virtual keeps Qt's name, because the signature is theirs:
...
evaluated and the verdict written here.**

The candidate is clang-format, and it is worth a real evaluation rather
than a reflexive no: `UseTab: AlignWithSpaces` describes precisely the
rule this project wants -- tabs for continuation and indentation, spaces
for alignment. Whether it *delivers* that everywhere is the open
question, and the places to check are the ones where alignment is
subtle: continuation parameters under an open paren, block comment `*`
columns, and aligned trailing comments.

Until somebody checks and records what happened, running it ad hoc on a
single file is the exact failure the source warns about -- a silent
...
The gate is `tools/style_gate.py`, spread verbatim from
`~/.claude/tools/style_gate.py`; its per-tree configuration is
`.style-gate.toml`, which explains why `Makefile` is deliberately not in
`indent_names`.
